...
public class CoachingSummaryASMPullActivity extends AppCompatActivity {
    private static final String TAG = "SummaryASMPullActivity";
    Button next;
        ...
        CoachingQuestionAnswerDAO.insertCoachingQA(coachingQAs, new CoachingQuestionAnswerDAO.InsertCoachingQAListener() {
            ...
            public void onInsertQuestionAnswerCompleted(boolean isSuccess) {
                if (isSuccess) {
                                            ...
                                            public void onGuidelineUpdated(boolean isSuccess) {
                                                PDFUtil.createPDF(coachingSessionID, new PDFUtil.GeneratePDFListener() {
                                                    ...
                                                    public void onPDFGenerated(boolean isSuccess) {
                                                        if (isSuccess) {
                                                            progressBar.dismiss();
                                                            SynchronizationService.sendEmail(coachingSessionID, CoachingSummaryASMPullActivity.this);
                                                        } else {
                                                            Toast.makeText(CoachingSummaryASMPullActivity.this, "Failed to generate PDF!!",
                    ...
                    Toast.makeText(CoachingSummaryASMPullActivity.this, "Failed to save the data!!!",
                            Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void saveQA() {
        addingQA("","asm_pull_summary_1",false,summary_1.getText().toString(),false);
        addingQA("","asm_pull_summary_2",false,summary_2.getText().toString(),false);
        addingQA("", "asm_pull_summary_3", false, summary_3.getText().toString(), false);
        CoachingSessionDAO.updateAction(coachingSessionID, summary_3.getText().toString(),
                new CoachingSessionDAO.UpdateCoachingListener() {
                    @Override
                    public void onGuidelineUpdated(boolean isSuccess) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if(requestCode == ConstantUtil.REQ_SEND_EMAIL){
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(CoachingSummaryASMPullActivity.this);
        ...
        public void onClick(DialogInterface dialog, int choice) {
            switch (choice) {
                case DialogInterface.BUTTON_POSITIVE:
                    ...
                    intent = new Intent(CoachingSummaryASMPullActivity.this, LoginActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
                    startActivity(intent);
                    break;
                case DialogInterface.BUTTON_NEGATIVE:
                    intent = new Intent(CoachingSummaryASMPullActivity.this, ProfileActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
                    intent.putExtra("coach", coach.getText().toString());
                    intent.putExtra("job", job);
