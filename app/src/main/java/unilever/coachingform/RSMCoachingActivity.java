...
public class RSMCoachingActivity extends AppCompatActivity {
    ...
    private void saveQA() {
        addingQA("","rsm_summary_1",false,summary_1.getText().toString(),false);
        addingQA("","rsm_summary_2",false,summary_2.getText().toString(),false);
        addingQA("", "rsm_summary_3", false, summary_3.getText().toString(), false);
        CoachingSessionDAO.updateAction(coachingSessionID, summary_3.getText().toString(),
                new CoachingSessionDAO.UpdateCoachingListener() {
                    @Override
                    public void onGuidelineUpdated(boolean isSuccess) {
            ...
            public void onInsertQuestionAnswerCompleted(boolean isSuccess) {
                if (isSuccess) {
                    if (isNetworkAvailable()) {
                                            ...
                                            public void onGuidelineUpdated(boolean isSuccess) {
                                                PDFUtil.createPDF(coachingSessionID, new PDFUtil.GeneratePDFListener() {
                                                    ...
                                                    public void onPDFGenerated(boolean isSuccess) {
                                                        if (isSuccess) {
                                                            progressBar.dismiss();
                                                            SynchronizationService.sendEmail(coachingSessionID, RSMCoachingActivity.this);
                                                        } else {
                                                            Toast.makeText(RSMCoachingActivity.this, "Failed to generate PDF!!",
                                                                    Toast.LENGTH_SHORT).show();
                                                        }
                                                    }
                                                });
                        ...
                        Toast.makeText(RSMCoachingActivity.this, "Your coaching from will be saved locally",
                                Toast.LENGTH_SHORT).show();
                        Intent intent = new Intent(RSMCoachingActivity.this, ProfileActivity.class);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if(requestCode == ConstantUtil.REQ_SEND_EMAIL){
            AlertDialog.Builder builder = new AlertDialog.Builder(RSMCoachingActivity.this);
        ...
        public void onClick(DialogInterface dialog, int choice) {
            switch (choice) {
                case DialogInterface.BUTTON_POSITIVE:
                    ...
                    intent = new Intent(RSMCoachingActivity.this, LoginActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
                    startActivity(intent);
                    break;
                case DialogInterface.BUTTON_NEGATIVE:
                    intent = new Intent(RSMCoachingActivity.this, ProfileActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
                    intent.putExtra("coach", coach.getText().toString());
                    intent.putExtra("job", job);
